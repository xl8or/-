...
public class DecoderUtil {
   ...
   public static boolean sendorReceive = 1;
   ...
   public static String decodeEncodedWords(String var0) {
      String var1;
      if(var0.indexOf("=?") == -1) {
         var1 = guessCharsetAndDecode(var0);
      } else {
         int var2 = 0;
         boolean var3 = false;
   ...
   private static CharsetDecoder getDecoder(String var0) {
      CharsetDecoder var1 = (CharsetDecoder)decoders.get(var0);
      if(var1 == null) {
         var1 = Charset.forName(var0).newDecoder();
         CodingErrorAction var2 = CodingErrorAction.REPORT;
         var1.onMalformedInput(var2);
         CodingErrorAction var4 = CodingErrorAction.REPORT;
         var1.onUnmappableCharacter(var4);
         decoders.put(var0, var1);
      }

      return var1;
   }

   static String guessCharsetAndDecode(String var0) {
      String var1;
      if(!guessingAllowed()) {
         sendorReceive = (boolean)1;
         var1 = var0;
      } else {
         ...
         if(var3) {
            ...
            int var8;
            for(byte var10 = 0; var10 < var2; var8 = var10 + 1) {
               byte var7 = (byte)var0.charAt(var10);
               var6[var10] = var7;
            }

            String var9 = guessCharsetAndDecode(var6);
            sendorReceive = (boolean)1;
            if(var9 != null) {
               var1 = var9;
            } else {
   ...
   static String guessCharsetAndDecode(byte[] var0) {
      ByteBuffer var1 = ByteBuffer.wrap(var0);

      String var2;
      String var3;
      try {
         var2 = getDecoder("ksc5601").decode(var1).toString();
      } catch (CharacterCodingException var11) {
         Buffer var5 = var1.rewind();

         try {
            var2 = getDecoder("UTF-8").decode(var1).toString();
         } catch (CharacterCodingException var10) {
            Buffer var7 = var1.rewind();

            try {
               var2 = getDecoder("UCS-2").decode(var1).toString();
   ...
   private static boolean guessingAllowed() {
      String var0 = Locale.getDefault().getLanguage();
      boolean var1;
      if("ko".equals(var0) && sendorReceive) {
         var1 = true;
      } else {
         var1 = false;
      }
